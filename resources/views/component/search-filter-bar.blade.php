...
<div id="searchFilterBar" class="sticky top-[72px] z-40 w-full transition-all duration-300">
    <div class="bg-[#d4bfaf] mx-auto shadow-lg" style="backdrop-filter: blur(10px);">
        ...
        <form action="{{ url('/property') }}" method="GET" class="max-w-6xl mx-auto px-4 md:px-6 py-3 md:py-5"
            x-data="{
                location: '{{ request('city') }}', locationOpen: false,
                type: '{{ request('type') }}', typeOpen: false,
                price: '{{ request('price') }}', priceOpen: false,
                mobileFilterOpen: false, // <-- State baru khusus buat HP
            ...
            {{-- 1. TOMBOL PEMANCING DI HP (Cuma muncul di layar kecil) --}}
            <button type="button" @click="mobileFilterOpen = !mobileFilterOpen" class="md:hidden w-full bg-white/90 text-[#5a3e2b] px-5 py-3.5 rounded-xl font-medium flex items-center justify-between shadow-sm">
                <div class="flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-[#8b6f5a]" viewBox="0 0 24 24" fill="currentColor">
                        <path fill-rule="evenodd" d="M10.5 3.75a6.75 6.75 0 1 0 0 13.5 6.75 6.75 0 0 0 0-13.5ZM2.25 10.5a8.25 8.25 0 1 1 14.59 5.28l4.69 4.69a.75.75 0 1 1-1.06 1.06l-4.69-4.69A8.25 8.25 0 0 1 2.25 10.5Z" clip-rule="evenodd" />
                    </svg>
                    <span x-text="hasFilter ? 'Filter Sedang Aktif' : 'Cari Properti...'"></span>
                </div>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 transition-transform duration-300" :class="mobileFilterOpen ? 'rotate-180' : ''" viewBox="0 0 24 24" fill="currentColor">
                    <path fill-rule="evenodd" d="M12.53 16.28a.75.75 0 0 1-1.06 0l-7.5-7.5a.75.75 0 0 1 1.06-1.06L12 14.69l6.97-6.97a.75.75 0 1 1 1.06 1.06l-7.5 7.5Z" clip-rule="evenodd" />
                </svg>
            </button>

            {{-- 2. WADAH INPUTAN (Disembunyiin di HP kecuali di-klik, tapi selalu nampil di Desktop) --}}
            <div class="flex-col md:flex-row md:items-center gap-3 md:gap-4 mt-3 md:mt-0 transition-all duration-300"
                 :class="mobileFilterOpen ? 'flex' : 'hidden md:flex'">
                ...
                <div class="relative flex-1 w-full">
                    ...
                    <button type="button" @click="typeOpen = !typeOpen" @click.outside="typeOpen = false"
                        ...
                        <span class="font-medium text-sm truncate" :class="type ? 'text-[#3d2b1f]' : 'text-[#5a3e2b]'" x-text="type || 'Tipe Properti'"></span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 text-[#8b6f5a] group-hover:text-[#5a3e2b] transition-colors shrink-0">
                ...
                <div class="relative flex-1 w-full">
                    ...
                    <button type="button" @click="priceOpen = !priceOpen" @click.outside="priceOpen = false"
                        ...
                        <span class="font-medium text-sm truncate" :class="price ? 'text-[#3d2b1f]' : 'text-[#5a3e2b]'" x-text="price || 'Rentang Harga'"></span>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5 text-[#8b6f5a] group-hover:text-[#5a3e2b] transition-colors shrink-0">
                ...
                <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto mt-2 md:mt-0">
                    <button type="button" x-show="hasFilter" style="display: none;"
                        @click="resetAll()"
                        class="w-full sm:w-auto flex justify-center items-center gap-1.5 bg-white/80 hover:bg-red-50 border border-red-300 text-red-600 hover:text-red-700 px-4 py-3.5 rounded-xl font-medium text-sm transition-all duration-200 hover:shadow-md active:scale-95 whitespace-nowrap">
                    ...
                    <button type="submit" class="w-full sm:w-auto flex justify-center items-center gap-2 bg-[#3d2b1f] hover:bg-[#2a1d14] text-white px-8 py-3.5 rounded-xl font-semibold text-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 active:scale-95 whitespace-nowrap">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
                            <path fill-rule="evenodd" d="M10.5 3.75a6.75 6.75 0 1 0 0 13.5 6.75 6.75 0 0 0 0-13.5ZM2.25 10.5a8.25 8.25 0 1 1 14.59 5.28l4.69 4.69a.75.75 0 1 1-1.06 1.06l-4.69-4.69A8.25 8.25 0 0 1 2.25 10.5Z" clip-rule="evenodd" />
                        </svg>
                        Cari
